...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        String subStr2 = str2.substring(overlap, str2.length());
        return str1 + subStr2;
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
         ...
         */
        //Declare a new set to store the string that satisfy the requirement
        Set<String> goodStr = new Set1L<>();
        //first check if the str is just a substring of one of the string in set
        boolean isSub = false;
        for (String setStr : strSet) {
            /*
             * if the substring situation is found, there will be no need to
             * change the initial set
             */
            if (setStr.indexOf(str) != -1) {
                isSub = true;
            }
        }
        if (!isSub) {
            //only strings that is not the substring of str is qulified to get
            // in the "goodStr" set
            for (String setStr : strSet) {
            ...
            //update strSet by "goodStr" and add the "str" in
            strSet.transferFrom(goodStr);
            strSet.add(str);
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        for (int i = 0; i < text.length(); i++) {
            ...
            if (text.charAt(i) != '~') {
                ...
                //the character is ~, start with a new line
                out.println();
            }
        }
